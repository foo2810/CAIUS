...
def get_arch_type(trial):
    arch = trial.suggest_categorical('arch', ['b0', 'b1', 'b2', 'b3', 'b4', 'b5', 'b6', 'b7'])
    return arch

def objective(trial):
    arch = get_arch_type(trial)
    if arch == 'b0':
        model = EfficientNetB0(in_shape, classes=n_classes)
    elif arch == 'b1':
        model = EfficientNetB1(in_shape, classes=n_classes)
    elif arch == 'b2':
        model = EfficientNetB2(in_shape, classes=n_classes)
    elif arch == 'b3':
        model = EfficientNetB3(in_shape, classes=n_classes)
    elif arch == 'b4':
        model = EfficientNetB4(in_shape, classes=n_classes)
    elif arch == 'b5':
        model = EfficientNetB5(in_shape, classes=n_classes)
    elif arch == 'b6':
        model = EfficientNetB6(in_shape, classes=n_classes)
    elif arch == 'b7':
        model = EfficientNetB7(in_shape, classes=n_classes)
